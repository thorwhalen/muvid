...
**But it does stop a render — the auto path now stops less of one.** The separate `unreliable` list names the clips whose *offset* the aligner will not vouch for. On the auto path the selection simply prefers other clips: a stretch of the song some other clip covers goes to that clip, and a stretch only an unreliable clip covers is left black and named in `coverage.excluded` with the clip and the numbers behind the verdict, so a five-clip shoot with one bad clip still assembles (muvid#88). `muvid_assemble_music_video` still refuses an explicit `edl` that names an unreliable clip, and still refuses when *no* clip is trustworthy — a black video reported as success is the same failure one level up (`allow_unreliable=true` overrides both, and also turns off the setting-aside, since opting in means you want that footage rendered). That refusal exists because a wrong offset is the one failure that does not look like one: it renders a complete, plausible video that is silently out of sync with the song, and nothing downstream re-measures it. On one real shoot three clips came back 83 s, 174 s and 83 s wrong, all reported with a low-but-not-alarming confidence, and the mistake only surfaced in the finished cut. Re-align, leave those clips out of the edit, or opt in on purpose — but do it knowingly.
