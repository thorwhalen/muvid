...
1. **One intermediate per PART** — a single-input ffmpeg run per cut (input-side `-ss`,
   ...
   A boundary carrying a [`Transition`](muvid.footage.edl.md#muvid.footage.edl.Transition) contributes a fourth
...
([`muvid.footage.look`](muvid.footage.look.md#module-muvid.footage.look)) spliced into the per-part chain by `_part_filter()`.
...
[`LOOK_FILTERS`](muvid.footage.edl.md#muvid.footage.edl.LOOK_FILTERS) names, and refuses a fragment that names a
...
([`look_time_varying`](muvid.footage.edl.md#muvid.footage.edl.EdlEntry.look_time_varying)) because the fragment is a
...
[`muvid.visualize.ffmpeg.run_ffmpeg()`](muvid.visualize.ffmpeg.md#muvid.visualize.ffmpeg.run_ffmpeg); note the guard is **per invocation** now, so a
...
[`assemble_music_video()`](muvid.mcp.footage_tools.md#muvid.mcp.footage_tools.assemble_music_video) is a live per-caller MCP
