...
[`FootageAlignment`](muvid.footage.edl.md#muvid.footage.edl.FootageAlignment)s (which clip covers which span of the
...
[`muvid.footage.edl.derive_cuts()`](muvid.footage.edl.md#muvid.footage.edl.derive_cuts) (SSOT), so a third-party strategy can’t desync the
cut. Every strategy’s output still passes [`validate_edl()`](muvid.footage.edl.md#muvid.footage.edl.validate_edl) before
...
[`vouches_for()`](muvid.footage.edl.md#muvid.footage.edl.vouches_for)’s. A third-party strategy that ranks clips itself
should carry the same preference; `weighted` does it as a reward penalty rather than a
special case (see [`muvid.footage.select_score`](muvid.footage.select_score.md#module-muvid.footage.select_score)).
...
+ beats declares a keyword-only `context` parameter (or `**kwargs`); [`select_edl()`](#muvid.footage.strategy.select_edl)
  then passes a `SelectionContext` (defined in [`muvid.footage.select_score`](muvid.footage.select_score.md#module-muvid.footage.select_score)) as
